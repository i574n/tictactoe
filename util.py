...
_node_cache = ''
_ipython_node_path = '_ipython_node.ts'
...
def node(arg, cell, test=False):
    ...
    write_file(_ipython_node_path, new_code_ipython)

    if arg == 'run':
        result = run(f'NODE_NO_WARNINGS=1 node --loader ts-node/esm --es-module-specifier-resolution=node {_ipython_node_path}').splitlines()
    else:
        result = []
